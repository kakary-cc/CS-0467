...
function game(config = null) {
    ...
    if (config === null) {
        ...
        ({deck: deck, hands: [computerHand, playerHand]} = cards.deal(deck));
    ...
    function displayState() {
        ...
        console.log('-----------------------------------------------');
        console.log('Next suit/rank to play: →  ' + cards.handToString([nextPlay]) + '  ←');
        console.log('-----------------------------------------------');
        console.log('Top of discard pile: ' + cards.handToString([discardPile[discardPile.length - 1]]));
    ...
    if (cards.playable(playerHand, nextPlay)) {
        ...
        let choice = question(cards.handToString(playerHand, '\n', true) + '\n> ');
        choice = parseInt(choice);
        if (choice > 0 && choice < playerHand.length + 1 && cards.playable([playerHand[choice - 1]], nextPlay)) {
            console.log('\nCard played: ' + cards.handToString([playerHand[parseInt(choice) - 1]]));
        ...
        console.log('Press ENTER to draw cards until matching: %s, %s, 8', nextPlay.rank, nextPlay.suit);
        question('');
        let drawn = [];
        [deck, drawn] = cards.drawUntilPlayable(deck, nextPlay);
        console.log('Cards drawn:' + cards.handToString(drawn));
        console.log('Cards played:' + cards.handToString([drawn[drawn.length - 1]]));
    ...
    if (cards.playable(computerHand, nextPlay)) {
        for (let i = 0; i < computerHand.length; i++) {
            ...
            if (cards.playable([computerHand[i]], nextPlay)) {
                console.log('Card played: ' + cards.handToString([computerHand[i]]));
